...
cmd:option('-stp_decay', 1e-2, 'weight decay of short term parameters.')
...
if opt.gf then name = "gf_" .. name
elseif opt.dg then name = "dg_" .. name end
...
local optim_state = { ltp={} }
if opt.stp_lr > 0 then optim_state.stp = {} end
...
if opt.overwrite then
  optim_state.losses = {}
  optim_state.ltp.learningRate = opt.lr
  -- ADAM
  optim_state.ltp.beta1 = opt.beta1
  optim_state.ltp.beta2 = opt.beta2
  -- SGD for STP
  if optim_state.stp then
    optim_state.stp.learningRate = opt.stp_lr
  end
...
local st_params, tmp_params, lt_params

if opt.overwrite then params:uniform(-0.08, 0.08) end
if optim_state.stp then
  st_params  = optim_state.st_parameters or params:clone():zero()
...
optim_state.valid_loss = 1000000
local total = 0
...
local function run_split(split_index)
  -- save current state, and set it to zero
  for k,l in pairs(decoder.layers) do
    if l.start_s then
      if type(l.start_s) == "table" then
  ...
  if optim_state.stp then 
    tmp_params:copy(st_params)
    st_params:zero()
    params:copy(lt_params)
  end
  -- run
  ...
  for i = 1, n do
    ...
    if optim_state.stp then
      ...
      end, st_params, optim_state.stp)
    end
    loss = loss + l
    if optim_state.stp then params:copy(lt_params):add(st_params) end
  end
...
local function run_checkpoint()
  ...
  if log then
    ...
    if optim_state.stp then params:add(-1,st_params) end
    decoder:save(model_file)
    if optim_state.stp then params:add(st_params) end

    optim_state.batch_ix = loader.batch_ix[1]
    optim_state.step = step
    optim_state.time = torch.toc(beginning_time)
    optim_state.st_parameters = st_params
    ...
    elseif optim_state.step/opt.checkpoint < step/opt.checkpoint - 2 then
      ...
      --drop learning rate
      optim_state.ltp.learningRate = optim_state.ltp.learningRate * opt.lr_decay
      print("No validation set improvement! Decaying learning rate to: " .. optim_state.ltp.learningRate)
    end
  end
end
...
while step < (opt.epochs * epoch_size) do
  ...
  local _, loss = optim.adam(feval, lt_params, optim_state.ltp)
  if optim_state.stp then
    optim.sgd(stp_eval, st_params, optim_state.stp)
    params:copy(lt_params):add(st_params)
  end
  ...
  if (step-1) % math.floor(epoch_size / 10 +0.5) == 0 or step % epoch_size == 0 then
    ...
    print('epoch = ' .. util.f3(epoch) ..
        ...
        ', wps = ' .. wps ..
        ', lr = ' .. string.format("%.4f", optim_state.ltp.learningRate) ..
...
if step % opt.checkpoint ~= 0 and optim_state.step ~= step then
  run_checkpoint()
end
